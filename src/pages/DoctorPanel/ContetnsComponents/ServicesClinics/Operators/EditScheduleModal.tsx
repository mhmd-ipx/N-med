...
import type { CreateSchedulesRequest, ScheduleData, ScheduleTime, Service, UserTime } from '../../../../../types/types';
import { updateSchedule, getServices, getUserTimes } from '../../../../../services/serverapi';
...
interface EditScheduleModalProps {
  ...
  isOpen: boolean;
  onClose: () => void;
  onScheduleUpdate: (clinicId: number, userId: number) => void;
  currentSchedules: UserTime[];
}
...
const EditScheduleModal: React.FC<EditScheduleModalProps> = ({
  ...
  useEffect(() => {
    ...
    const prepareSchedules = () => {
      const groupedSchedules: { [key: string]: ScheduleTime[] } = {};

      currentSchedules
        .filter(time => time.clinic_id === clinicId)
        .forEach(time => {
          if (!groupedSchedules[time.weekday]) {
            groupedSchedules[time.weekday] = [];
          }
          groupedSchedules[time.weekday].push({
            clinic_id: clinicId,
      ...
      const preparedSchedules = Object.entries(groupedSchedules).map(([weekday, times]) => ({
        weekdays: [weekday],
        times: times
      }));

      setSchedules(preparedSchedules);
      setSelectedDays(Object.keys(groupedSchedules));
    };

    loadServices();
    prepareSchedules();
  }, [currentSchedules, clinicId]);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const response = await updateSchedule(clinicId, payload);
      ...
      onScheduleUpdate(clinicId, userId);
      onClose();
    } catch (error) {
      setErrorMessage(error instanceof Error ? error.message : 'خطا در ویرایش زمان‌بندی');
    } finally {
      setIsSubmitting(false);
    }
  };
    ...
    <div className="fixed inset-0 z-[1000] flex items-center justify-center bg-black bg-opacity-25">
      <div className="w-full max-w-2xl h-[85vh] flex flex-col rounded-2xl bg-white text-right shadow-xl overflow-hidden">
        <div className="p-6 pb-0">
          <h3 className="text-lg font-medium text-gray-900">ویرایش زمان‌بندی</h3>
        </div>
        <style>
          {`
            .accordion-content {
        ...
        <div className="flex-1 overflow-y-auto p-6">
          {schedules.map((schedule, index) => (
            <div key={index} className="border rounded-lg mb-4 overflow-auto">
              <button
                type="button"
